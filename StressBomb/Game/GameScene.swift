...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        binEmpty.size = CGSize(width:(bin.size.width)*(self.size.width/bin.size.width)/(11.42/2.3),  height: bin.size.height * (self.size.height/bin.size.height)/(6.42/3.2))
        ...
        for row in 1...5{
            ...
            for col in 0...4{
                ...
                print("numeroRandom", numeroRandom)
    ...
    func Checar(){
        ...
        if secondBody.node?.name == myRandomItem.name {
            
            positionContainer = containerSprite.index(where: { $0.block.name == secondBody.node?.name })!
        }else{
            myRandomItem.position = CGPoint(x: self.frame.size.width / -4, y: self.frame.size.width / 3)
        }
        ...
        if firstBody.node?.name == "Container_Square_DBlue" && secondBody.node?.name == figure_name_sqdb  {
            print("Contact detected")
            containerSprite[positionContainer].block.texture  = SKTexture(imageNamed: "Square_DBlue_Filled")
            //print("Contact happened between square 1 and square 2")
